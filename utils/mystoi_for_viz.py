...
def compute_stoi(clean_audio, spin_audio, sampling_rate: int, return_sfr=False, return_stft=False, return_octave_stft=False) -> float:
    """
...
def remove_silent_frames(clean_audio, spin_audio, dyn_range, true_frame_len, overlap):
    ...
    # mask = (np.max(clean_energies) - dyn_range - clean_energies) < 0 # from pystoi
